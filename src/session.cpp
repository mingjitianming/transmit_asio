...
Session::Session(asio::io_context &io_context, const std::shared_ptr<std::map<DataHeader, std::shared_ptr<Transmit>>> &methods)
    : socket_(io_context),
...
std::shared_ptr<Session> Session::create(asio::io_context &io_context, const std::shared_ptr<std::map<DataHeader, std::shared_ptr<Transmit>>> &methods)
{
    //TODO: 修改为共享
    return std::shared_ptr<Session>(new Session(io_context, methods));
}

std::shared_ptr<Transmit> Session::getMethod(const DataHeader &method_header)
{
